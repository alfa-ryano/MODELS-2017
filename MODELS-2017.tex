...
\begin{abstract}
\label{abstract}
Backgrounded by the success of the applications of gamified approaches in different fields, this research harnesses the engaging nature of games, the effectiveness of pedagogy, and the automation of Model-driven Engineering to propose a framework for model-driven gamified software modelling learning. It is a framework for tutors to create software modelling learning patterns, which later can be transformed to generate software modelling learning games for learners to play with. This paper presents the motivation behind the initiation of the framework as well as the problem analysis and the solution overview in realising the framework. The framework is then demonstrated to show how the framework works in producing a game that teach the concept of substate in statechart modelling. Two forms of assessments are then presented as the evaluation for the framework.
\end{abstract}
